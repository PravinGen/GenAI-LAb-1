...
--  File 02: Stored Procedures & Triggers (FIXED)
-- =============================================================
...
CREATE PROCEDURE issue_book (
    ...
    IN  p_borrow_days   TINYINT UNSIGNED,
    OUT p_txn_id        BIGINT UNSIGNED,
    OUT p_message       VARCHAR(255)
)
issue_book_proc: BEGIN
    ...
    IF v_outstanding > 0 THEN
        SET p_message = CONCAT('ERROR: User has outstanding fine of Rs.', v_outstanding);
        LEAVE issue_book_proc;
    END IF;
    ...
    SET v_due_date = DATE_ADD(CURRENT_DATE,
                     INTERVAL IF(p_borrow_days > 0, p_borrow_days, v_policy_days) DAY);
...
CREATE PROCEDURE return_book (
    ...
    IN  p_condition_return  VARCHAR(20),
...
return_proc: BEGIN
    ...
    SELECT user_type INTO v_user_type FROM users WHERE user_id = v_user_id;
    ...
    IF v_total_fine > 0 AND v_fine_type IS NOT NULL THEN
        INSERT INTO fines (txn_id, user_id, fine_type, days_overdue,
                           base_amount, final_amount, raised_on, status)
        VALUES (p_txn_id, v_user_id, v_fine_type, v_days_late,
                v_total_fine, v_total_fine, CURRENT_DATE, 'pending');
    ...
    SET p_message = CONCAT('SUCCESS: Book returned. Fine: Rs.', COALESCE(v_total_fine, 0));
END$$
...
pay_proc: BEGIN
    ...
    SET p_message = CONCAT('SUCCESS: Payment of Rs.', p_amount, ' recorded.');
END$$
...
-- =============================================================
--  TRIGGER: mark overdue on update
-- =============================================================
...
-- =============================================================
--  EVENT: nightly overdue scan
-- =============================================================
CREATE EVENT IF NOT EXISTS daily_overdue_scan
ON SCHEDULE EVERY 1 DAY
STARTS CONCAT(DATE(NOW() + INTERVAL 1 DAY), ' 00:05:00')
DO
    UPDATE book_transactions
    SET    status = 'overdue'
    WHERE  status   = 'issued'
